...
export class DigitalPaymentReceipt extends React.Component<IDigitalPaymentReceiptProps,any>
{
    ...
    generateReceipts = () =>
    {
        ...
        printDoc.setFontSize(11).setFont(printDoc.getFont().fontName, 'normal')
        printDoc.text(this.props.basicInfoData.FirstName+" "+this.props.basicInfoData.LastName,400,215)
        printDoc.text(this.props.basicInfoData.Contact,400,230)
        printDoc.text(this.props.basicInfoData.Address,400,245,{maxWidth:180})

        const transactionColumns = ['Sr. No','Payment Date','Receipt No.','Transaction No.','Payment Mode','Paid Amount'];
        ...
        //const trowHeight = 10;
        const footerData1 = ['', '', '', '', 'Total Fees Paid:', this.props.emiInfoData.feesPaid];
        const footerData2 = ['', '', '', '', 'Total Course Fees:', this.props.courseInfo.totalFees];
        const footerData3 = ['', '', '', '', 'Remaining Fees:', this.props.emiInfoData.totalRemainingAmount];
        let footerData:any = [footerData1,footerData2,footerData3]

        this.props.transactionInfoData.sort((a:any, b:any) => a.paymentDate - b.paymentDate);    
        ...
        autoTable(printDoc,{
            ...
            headStyles: { fillColor: [218, 138, 62],fontSize:11,textColor:[255,255,255] }, // Header background color
            bodyStyles: { textColor: [0, 0, 0], fontSize: 10 },
            theme: 'striped', // 'striped', 'grid', or 'plain'
            styles: { overflow: 'linebreak' },
            foot:footerData,
            footStyles: { fillColor: [255, 255, 255],fontSize:10,textColor:[1,1,1],halign:'center' },
            //startY: ty + trowHeight, // Move the table down
          });

        
        const emiColumns = ['Sr. No','EMI Date','EMI Amt','Principal Paid','Remaining Principal','EMI Status'];
